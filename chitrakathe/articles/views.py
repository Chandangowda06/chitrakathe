...
from articles.models import Members, Post, Image, Category
        ...
        if request.user.is_staff:
            if request.method == "POST":
                ...
                if first_name and last_name and email and phone and profile_image:
                    member = Members.objects.create(first_name=first_name, last_name=last_name, profession=profession, email=email, phone=phone, instagram_link=instagram_link, facebook_link=facebook_link, twitter_link=twitter_link, linkedin_link=linkedin_link, profile_image=profile_image, location=location)
                    if member:
                        return redirect('/admin-members/')
                else:
                    return render(request, 'admin1/add_member.html',{'error_message':'please enter all details'})
            return render(request, 'admin1/add_member.html', {'authors': Members.objects.all(), 'categories': Category.objects.all()})
        return HttpResponse("you are not admin to access this page")
    raise Http404("Page not found")
...
def edit_member(request, member_id):
    ...
    if authenticated:
        if request.user.is_staff:
            ...
            print(member.id)
            return render(request, 'admin1/edit_member.html', {'member': member})
